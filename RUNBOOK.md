...
**Scope:** any single-developer APEX 26.1 application. This is the template — copy it into each project's repo as `README.md` (or keep one copy and just follow it), fill in the three placeholders, done. The MCC platform has its own team runbook; none of that ceremony applies here.
...
**Drop a `CLAUDE.md` in the repo root** with the house rules. A full worked sample ships alongside this runbook (`CLAUDE.md` — MARAD-flavored; adjust names and IDs per project). The minimum viable version:
