...
- **Grants don't cover future objects.** Refresh them after adding tables — but with `db/refresh-claude-ro-grants.sql`, the promptless twin. The password prompt lives only in `create-claude-ro.sql` (creation is a one-time, human-at-a-terminal act); the refresh script asks nothing, which is what lets `migrate.sh` run it automatically after every migration batch without looking hung at a hidden prompt.
...
**Tier 1 — hand-rolled (fine for a small, slow-moving schema):** `db/` holds current-state `CREATE OR REPLACE` sources; `db/migrations/YYYYMMDD-nn-*.sql` holds ordered, run-once DDL/data changes. You are the tracking system. Migration runs before the app change that needs it — via **`scripts/migrate.sh <file> [<file> ...] [ADMIN_CONN]`** (human-only, like push): it runs the files in order, stops at the first failure, then refreshes CLAUDE_RO's grants **once at the end** via the promptless `db/refresh-claude-ro-grants.sql` — the forget-prone step that otherwise leaves the agent blind to new tables, now with no hidden password prompt to mistake for a hang. Conventions: `db/migrations/README.md`.
